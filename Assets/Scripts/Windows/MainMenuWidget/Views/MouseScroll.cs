...
namespace Windows.MainMenuWidget.Views
{
	...
	public class MouseScroll : MonoBehaviour
	{
		...
		void Update()
		{
			if (!m_OverrideScrolling || m_ScrollRect == null || m_ScrollRectTransform == null)
			{
				return;
			}
			...
			Vector2 localPoint;
			if (!RectTransformUtility.ScreenPointToLocalPointInRectangle(m_ScrollRectTransform, mousePosition, null, out localPoint))
			{
				return;
			}
		...
		{
			if (levelSelectButton == null || levelSelectButton.transform.parent == null || m_ScrollRect == null)
			{
				return;
			}
			...
			int siblingIndex = levelSelectButton.transform.GetSiblingIndex();
			if (siblingIndex < 0 || siblingIndex >= childCount)
			{
				return;
			}

			float normalized = (float)siblingIndex / (childCount - 1);
			normalized = Mathf.Clamp01(normalized); // Ensure normalized value is between 0 and 1
