...
namespace Utils
{
	...
	public class Pool<T>
    {
        ...
        {
            if (m_Available.Count == 0) Grow(1);
            if (m_Available.Count == 0) throw new InvalidOperationException("Failed to grow pool");

            var itemIndex = m_Available.Count - 1;
            var item = m_Available[itemIndex];
            m_Available.RemoveAt(itemIndex);

            if (resetOverride != null) resetOverride(item);
	    ...
	    public virtual void Return(T pooledItem)
        {
            if (m_All.Contains(pooledItem) &&
                ...
                throw new InvalidOperationException("Trying to return an item to a pool that does not contain it: " +
                                                    pooledItem +
                                                    ", " + this);
        }
	    ...
	    public virtual void ReturnAll(Action<T> preReturn)
        {
            for (var i = 0; i < m_All.Count; ++i)
            {
                ...
                {
                    if (preReturn != null) preReturn(item);
	...
	public class AutoGameObjectPrefabPool : GameObjectPool
    {
        ...
        {
            var newElement = Object.Instantiate(m_Prefab);
            if (m_Initialize != null) m_Initialize(newElement);
