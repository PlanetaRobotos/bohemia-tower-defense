...
namespace Features.Camera
{
    public class RadiusVisualizerController : MonoBehaviour
    {
        ...
        private readonly List<GameObject> m_RadiusVisualizers = new();
        ...
        public void SetupRadiusVisualizers(Tower tower, Transform ghost = null)
        {
            ...
            var length = providers.Count;
            for (var i = 0; i < length; i++)
            {
                if (m_RadiusVisualizers.Count < i + 1) m_RadiusVisualizers.Add(Instantiate(radiusVisualizerPrefab));

                var provider = providers[i];

                var radiusVisualizer = m_RadiusVisualizers[i];
                ...
                if (visualizerRenderer != null) visualizerRenderer.material.color = provider.effectColor;
            }
        }
        ...
        {
            foreach (var radiusVisualizer in m_RadiusVisualizers)
            {
