...
namespace TowerDefense.Targetting
{
	...
	public class Targetter : MonoBehaviour
    {
	    ...
	    /// </summary>
	    public Vector2 turretXRotationRange = new(0, 359);
	    ...
	    /// </summary>
	    protected List<Targetable> m_TargetsInRange = new();
	    ...
	    protected virtual void OnUpdate(float _)
        {
            ...
            if (m_SearchTimer <= 0.0f && m_CurrrentTargetable == null && m_TargetsInRange.Count > 0)
            {
                ...
                {
                    if (acquiredTarget != null) acquiredTarget(m_CurrrentTargetable);
                    m_SearchTimer = searchRate;
                }
            }
	    ...
	    protected virtual void OnTriggerEnter(Collider other)
        {
            ...
            if (targetEntersRange != null) targetEntersRange(targetable);
        }
	    ...
	    protected virtual void OnTriggerExit(Collider other)
        {
            ...
            if (targetExitsRange != null) targetExitsRange(targetable);
	    ...
	    protected virtual bool IsTargetableValid(Targetable targetable)
        {
            ...
            var targetAlignment = targetable.configuration.alignmentProvider;
            var canDamage = alignment == null || targetAlignment == null ||
        ...
        {
            var length = m_TargetsInRange.Count;
            ...
            {
                var targetable = m_TargetsInRange[i];
	    ...
	    private void OnTargetRemoved(DamageableBehaviour target)
        {
            ...
            {
                for (var i = 0; i < m_TargetsInRange.Count; i++)
	    ...
	    protected virtual void AimTurret()
        {
            ...
            if (m_CurrrentTargetable == null) // do idle rotation
            {
                if (m_WaitTimer > 0)
                {
                    ...
                    if (m_WaitTimer <= 0) m_CurrentRotationSpeed = (Random.value * 2 - 1) * idleRotationSpeed;
                }
                else
                {
                    var euler = turret.rotation.eulerAngles;
                    euler.x = Mathf.Lerp(Wrap180(euler.x), 0, m_XRotationCorrectionTime);
                    m_XRotationCorrectionTime =
                        Mathf.Clamp01((m_XRotationCorrectionTime + Time.deltaTime) / idleCorrectionTime);
            ...
            else
            {
                ...
                var targetPosition = m_CurrrentTargetable.position;
                if (onlyYTurretRotation) targetPosition.y = turret.position.y;
