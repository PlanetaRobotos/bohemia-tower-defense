...
namespace Features.Agents
{
    ...
    {
        protected List<float> m_CurrentEffects = new();
            ...
            float scale = 1)
        {
            ...
            var min = slowFactor;
            foreach (var item in m_CurrentEffects) min = Mathf.Min(min, item);

            var originalSpeed = m_Agent.originalMovementSpeed;
        ...
        public void RemoveSlow(float slowFactor)
        {
            ...
            if (m_CurrentEffects.Count != 0) return;
        ...
        {
            if (m_Agent != null) m_Agent.navMeshNavMeshAgent.speed = m_Agent.originalMovementSpeed;
            if (m_SlowFx != null)
            {
