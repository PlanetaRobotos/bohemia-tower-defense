...
namespace Features.Towers.Data
{
	/// <summary>
    ...
    public class TowerLibrary : ScriptableObject, IList<Tower>, IDictionary<string, Tower>
    {
	    ...
	    public Tower this[int index] => configurations[index];
        ...
        {
            get => m_ConfigurationDictionary[key];
            set => m_ConfigurationDictionary[key] = value;
        }

        public ICollection<string> Keys => ((IDictionary<string, Tower>)m_ConfigurationDictionary).Keys;

        ICollection<Tower> IDictionary<string, Tower>.Values => m_ConfigurationDictionary.Values;
        ...
        public void CopyTo(KeyValuePair<string, Tower>[] array, int arrayIndex)
        {
            ...
            {
                var config = configurations[i - arrayIndex];
                var current = new KeyValuePair<string, Tower>(config.towerName, config);
                array[i] = current;
            }
        }
        ...
        public bool IsReadOnly => ((ICollection<Tower>)configurations).IsReadOnly;
