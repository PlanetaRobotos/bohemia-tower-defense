...
using Features.Towers.Placement;
using UnityEngine;
using Utils;
...
namespace Features.Towers
{
	...
	public class Tower : Targetable
    {
	    ...
	    /// </summary>
	    public TowerPlacementGhost towerGhostPrefab => levels[currentLevel].towerGhostPrefab;
	    ...
	    /// </summary>
	    public int purchaseCost => levels[0].cost;
	    ...
	    public int GetSellLevel(int level)
        {
            ...
            {
                var cost = 0;
                for (var i = 0; i <= level; i++) cost += levels[i].cost;
        ...
        {
            if (level < 0 || isAtMaxLevel || level >= levels.Length) return false;
            SetLevel(level);
            return true;
        }
        ...
        protected void SetLevel(int level)
        {
            ...
            currentLevel = level;
            if (currentTowerLevel != null) Destroy(currentTowerLevel.gameObject);
            ...
            // disable affectors
            var levelState = LevelManager.levelState;
        ...
        protected virtual void ScaleHealth()
        {
            ...
            {
                var currentHealth = Mathf.FloorToInt(configuration.normalisedHealth * currentTowerLevel.maxHealth);
