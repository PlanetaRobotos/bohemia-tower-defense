...
namespace Features.Input.Declaration
{
	...
	public abstract class CameraInputScheme : InputScheme
    {
	    ...
	    protected void PanWithScreenCoordinates(Vector2 screenPosition, float screenEdgeThreshold, float panSpeed)
        {
            ...
            {
                var panAmount = (screenEdgeThreshold - screenPosition.x) / screenEdgeThreshold;
            ...
            {
                var panAmount = (screenEdgeThreshold - Screen.width + screenPosition.x) / screenEdgeThreshold;
                panAmount = Mathf.Clamp01(Mathf.Log(panAmount) + 1);
            ...
            {
                var panAmount = (screenEdgeThreshold - screenPosition.y) / screenEdgeThreshold;
                panAmount = Mathf.Clamp01(Mathf.Log(panAmount) + 1);
            ...
            {
                var panAmount = (screenEdgeThreshold - Screen.height + screenPosition.y) / screenEdgeThreshold;
                panAmount = Mathf.Clamp01(Mathf.Log(panAmount) + 1);
