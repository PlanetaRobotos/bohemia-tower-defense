...
namespace Features.Projectiles
{
    ...
    public class BallisticProjectile : MonoBehaviour, IProjectile
    {
        ...
        protected List<Collider> m_CollidersIgnoring = new();
        protected float m_CollisionIgnoreCount;
        ...
        protected virtual void Update()
        {
            ...
            if (m_IgnoringCollsions)
            {
                ...
                if (m_CollisionIgnoreCount >= collisionIgnoreTime)
                {
                    ...
                    foreach (var colliderIgnoring in m_CollidersIgnoring)
                    foreach (var projectileCollider in m_Colliders)
        ...
        ///     Automatically sets firing angle to suit launch speed unless angle is overridden, in which case launch speed is
        ///     overridden to suit angle.
        /// </summary>
        ...
        public virtual void FireAtPoint(Vector3 startPoint, Vector3 targetPoint)
        {
            ...
            switch (fireMode)
            {
                case BallisticFireMode.UseLaunchSpeed:
                    firingVector =
                        Ballistics.CalculateBallisticFireVectorFromVelocity(startPoint, targetPoint, startSpeed,
                            arcPreference);
                    firingAngle =
                        Ballistics.CalculateBallisticFireAngle(startPoint, targetPoint, startSpeed, arcPreference);
                    break;
                case BallisticFireMode.UseLaunchAngle:
                    firingVector =
                        Ballistics.CalculateBallisticFireVectorFromAngle(startPoint, targetPoint, firingAngle);
        ...
        public void IgnoreCollision(Collider[] collidersToIgnore)
        {
            if (collisionIgnoreTime > 0)
            {
                ...
                {
                    if (m_CollidersIgnoring.Contains(colliderToIgnore)) continue;
                    foreach (var projectileCollider in m_Colliders)
