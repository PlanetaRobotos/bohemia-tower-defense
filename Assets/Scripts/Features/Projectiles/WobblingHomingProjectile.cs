        ...
        protected override void OnUpdate(float _)
        {
            ...
            switch (m_State)
            {
                ...
                case State.Turning:
                    ...
                    m_Rigidbody.rotation = Quaternion.Lerp(m_Rigidbody.rotation, aimDirection,
                        m_CurrentTurnTime / turningTime);
                    m_Rigidbody.linearVelocity = transform.forward * m_Rigidbody.linearVelocity.magnitude;

                    if (m_CurrentTurnTime >= turningTime) m_State = State.Targeting;
                    break;
            }
        }
