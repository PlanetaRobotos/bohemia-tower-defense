...
namespace Features.Agents
{
	...
	public class AttackingAgent : Agent
    {
        ...
        {
            base.Remove();
            if (m_TargetTower != null) m_TargetTower.removed -= OnTargetTowerDestroyed;
            m_AttackAffector.enabled = false;
            m_TargetTower = null;
        }
        ...
        {
            base.LazyLoad();
            if (m_AttackAffector == null) m_AttackAffector = GetComponent<AttackAffector>();
        }
        ...
        protected override void OnPartialPathUpdate()
        {
            ...
            var distanceToTower = Vector3.Distance(transform.position, m_TargetTower.transform.position);
            if (!(distanceToTower < m_AttackAffector.towerTargetter.effectRadius)) return;
