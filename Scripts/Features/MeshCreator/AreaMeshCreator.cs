...
namespace Features.MeshCreator
{
    ...
    {
        [HideInInspector] public MeshObject meshObject;
	    ...
	    public Transform[] pointsTransforms
        {
            get
            {
                var childern = new Transform[pointsCenter.childCount];
                var length = pointsCenter.childCount;
                for (var i = 0; i < length; i++) childern[i] = pointsCenter.GetChild(i);
                return childern;
            }
        }
#endif
        ...
        {
            var count = pointsCenter.childCount;
            for (var i = 0; i < count - 1; i++)
            {
                var from = pointsCenter.GetChild(i).position;
                var to = pointsCenter.GetChild(i + 1).position;
                Gizmos.DrawLine(from, to);
            }

            // last to first
            var last = pointsCenter.GetChild(count - 1).position;
            var first = pointsCenter.GetChild(0).position;
        ...
        private List<Vector3> GetChildrenPositions(Transform parent)
        {
            ...
            for (var i = 0; i < length; i++) points.Add(parent.GetChild(i).position);
            return points;
        }
    }
    ...
    public class MeshObject
    {
        ...
        {
            var randomTriangle = triangles.WeightedSelection(completeArea, t => t.area);
            ...
            var cartesianPoint = randomBaryCentricPoint.x * randomTriangle.v0 + randomBaryCentricPoint.y *
                                 randomTriangle.v1 +
