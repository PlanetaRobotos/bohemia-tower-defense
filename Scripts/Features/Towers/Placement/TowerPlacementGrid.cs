...
namespace Features.Towers.Placement
{
	/// <summary>
    ...
    public class TowerPlacementGrid : MonoBehaviour, IPlacementArea
    {
	    ...
	    public Vector3 GridToWorld(IntVector2 gridPosition, IntVector2 sizeOffset)
        {
            ...
            var localPos = new Vector3(gridPosition.x + sizeOffset.x * 0.5f, 0, gridPosition.y + sizeOffset.y * 0.5f) *
                           gridSize;
	    ...
	    public TowerFitStatus Fits(IntVector2 gridPos, IntVector2 size)
        {
            ...
            if (size.x > dimensions.x || size.y > dimensions.y) return TowerFitStatus.OutOfBounds;
            ...
            for (var y = gridPos.y; y < extents.y; y++)
            for (var x = gridPos.x; x < extents.x; x++)
	    ...
	    public void Occupy(IntVector2 gridPos, IntVector2 size)
        {
            ...
            for (var x = gridPos.x; x < extents.x; x++)
            {
                ...
                if (m_Tiles != null && m_Tiles[x, y] != null) m_Tiles[x, y].SetState(PlacementTileState.Filled);
            }
        }
	    ...
	    public void Clear(IntVector2 gridPos, IntVector2 size)
        {
            ...
            for (var x = gridPos.x; x < extents.x; x++)
            {
                ...
                // If there's a placement tile, clear it
                if (m_Tiles != null && m_Tiles[x, y] != null) m_Tiles[x, y].SetState(PlacementTileState.Empty);
            }
        }
        ...
        {
            var tileToUse = placementTilePrefab;
            ...
            if (tileToUse != null)
            {
                ...
                for (var x = 0; x < dimensions.x; x++)
                {
                    ...
                    targetPos.y += 0.01f;
                    var newTile = Instantiate(tileToUse);
	    ...
	    private void OnDrawGizmos()
        {
            ...
            var center = transform.TransformPoint(new Vector3(gridSize * dimensions.x * 0.5f,
                1,
