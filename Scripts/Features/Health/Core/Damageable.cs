...
namespace Features.Health.Core
{
    ...
    public class Damageable
    {
        ...
        public void SetHealth(float health)
        {
            ...
            if (healthChanged != null) healthChanged(info);
        }
        ...
        public bool TakeDamage(float damage, IAlignmentProvider damageAlignment, out HealthChangeInfo output)
        {
            ...
            var canDamage = damageAlignment == null || alignmentProvider == null ||
                            damageAlignment.CanHarm(alignmentProvider);

            if (isDead || !canDamage) return false;

            ChangeHealth(-damage, output);
            SafelyDoAction(damaged, output);
            if (isDead) SafelyDoAction(died, output);
            return true;
        }
        ...
        public HealthChangeInfo IncreaseHealth(float health)
        {
            ...
            if (isAtMaxHealth) SafelyDoAction(reachedMaxHealth);
